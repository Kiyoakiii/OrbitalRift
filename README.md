...
Стартовая сцена уже добавлена. Expedition HUD теперь находится в редактируемом uGUI Canvas прямо в `Boot.unity`;
остальные экраны продолжают поэтапно переноситься с прежнего renderer. Инструкция по самостоятельной раскладке:
...
- `Assets/Scripts/CoopSimulationBridge.cs`, блок `CoopTetherRules` — дистанцию включения сцепки, натяжение, притяжение, толщину зоны урона, перегрев и cooldown повторного соединения.
- `Assets/Scripts/CoopSimulationBridge.cs`, блок `CoopFriendlyRedirectRules` — точный и магнитный радиусы перехвата союзной пули, усиление рикошета и величину безопасного пинбольного отскока.
