...
- MMR сохраняется вместе с рекордом и синхронизируется между устройствами;
- на стартовом экране показываются отдельные топ-5 по рекорду и MMR;
- игра остаётся полностью играбельной офлайн: сетевые ошибки не блокируют меню и полёт.

Если соединение пропало в момент окончания забега, результат сохраняется в
`PlayerPrefs` и повторно отправляется каждые 8 секунд. Очередь переживает полное
закрытие приложения и удаляется только после успешной записи Firestore.
...
3. Загружает личный лучший результат и оба публичных топ-5.
4. Записывает лучший результат и MMR игрока в Firestore после окончания игры.
...
Для развёртывания используйте основной файл `firebase/firestore.rules` и инструкцию `FIREBASE_PRODUCTION.md`. Файл `Docs/firestore.rules` оставлен как синхронизированная копия для ручной вставки в консоль. Правила дают всем устройствам чтение топа, но позволяют записать только собственный документ после анонимной авторизации, проверяют формат полей и ограничивают изменение MMR диапазоном текущей игровой формулы.
